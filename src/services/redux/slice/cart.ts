...
const slice = createSlice({
   ...
   reducers: {
      ...
      clearCartAction: (state) => {
         state.cart = [] as TCartItem[];
         // setLocalStorage("cart", state.cart);
      },
   },
});

export const { addToCartAction, incrementQuantityAction, decrementQuantityAction, removeFromCartAction, clearCartAction } = slice.actions;
