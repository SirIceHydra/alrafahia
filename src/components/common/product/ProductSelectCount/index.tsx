...
const ProductSelectCount: FC<TProps> = ({ className, id }): JSX.Element | false => {
   ...
   return (
      <div
         className={cn(
            "flex w-1/3 items-center justify-between overflow-hidden rounded-none bg-transparent *:flex *:items-center *:justify-center *:py-0 gap-2",
            className,
         )}
      >
         <button onClick={() => dispatch(decrementQuantityAction(id))} className="transition-opacity hover:opacity-80">
            <Image src={minus} alt="decrement" width={10} height={10} className="size-3" />
         </button>

         <span>{quantity}</span>

         <button onClick={() => dispatch(incrementQuantityAction(id))} className="transition-opacity hover:opacity-80">
